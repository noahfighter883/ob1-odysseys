    ...
    <>
      <section className={`text-white ${odysseyThemeStyles[odyssey.theme]}`}>
        <Container className="py-16">
          ...
          <p className="mt-4 max-w-2xl text-lg text-white/85">{odyssey.hook}</p>
        </Container>
      </section>
      ...
      {isComingSoon ? (
        <Container className="py-16">
          <div className="max-w-2xl rounded-2xl border border-border bg-card p-8">
            ...
            <p className="mt-3 text-muted-foreground">{odyssey.comingSoonNote}</p>
            <Button
              className="mt-6"
              nativeButton={false}
      ...
      <div className="grid gap-16 lg:grid-cols-[minmax(0,1fr)_260px]">
        <div className="space-y-16">
            ...
            <div className="mt-8 rounded-2xl border border-border bg-card p-6">
              ...
              <ul className="mt-4 divide-y divide-border text-sm">
                {analysis.largestFires.map((fire) => (
                  <li key={fire.id} className="flex items-center justify-between gap-4 py-3">
                    <div>
          ...
          <section id="datasets" className="scroll-mt-24">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              {odyssey.datasets?.map((dataset) => (
                <div key={dataset.file} className="flex flex-col rounded-2xl border border-border bg-card p-5">
          ...
          {/* Video */}
          <section id="video" className="scroll-mt-24 rounded-2xl border border-border bg-secondary/40 p-6">
            <p className="font-heading font-semibold">Watch the PSA</p>
            <p className="mt-2 text-sm text-muted-foreground">
              The short-form video for this odyssey is published on Instagram.
              The full data case above is what backs every claim in it.
            </p>
            <a
