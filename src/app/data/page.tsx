  ...
  return (
    <Container className="py-16">
      ...
      <section className="mt-14">
        <h2 className="font-heading text-2xl font-semibold">Our principles</h2>
      ...
      <section className="mt-16">
        ...
        <p className="mt-3 max-w-2xl text-sm text-muted-foreground">
          Datasets are added as each odyssey&apos;s field research and analysis
          are completed — this list grows with the program.
        </p>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          {datasets.map((dataset) => (
            <div key={dataset.file} className="flex flex-col rounded-2xl border border-border bg-card p-6">
              <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
                {dataset.odyssey.title}
              </p>
              <p className="mt-1 font-heading font-semibold">{dataset.label}</p>
      ...
      <section className="mt-16 rounded-2xl border border-border bg-secondary/40 p-8">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          ...
          source agencies (CAL FIRE, USFS, NIFC, and others) update their own
          public records. Each dataset&apos;s methodology notes call out its own
          known gaps and recommended next steps for higher-precision
          verification.
        </p>
      </section>
    </Container>
